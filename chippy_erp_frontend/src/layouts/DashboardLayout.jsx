...
import { LayoutDashboard, Users, CreditCard, Box, Map, Bell, LogOut, ChevronDown, UserCircle, ShoppingCart, UserCheck, BarChart3, ShieldCheck, X, Menu, Home, CalendarDays, MoreHorizontal } from 'lucide-react';
import { jwtDecode } from 'jwt-decode';
import logo from '../assets/image-removebg-preview.png';
import { usePermissions } from '../hooks/usePermissions.js';

// ─── sidebar menu definition (static — defined outside component) ────────────
const MENU_SECTIONS = [
  {
    title: 'SALES',
    items: [
...
export default function DashboardLayout() {
  ...
  // Single source of truth for permissions — from PermissionContext
  ...
  useEffect(() => {
    ...
    for (const section of MENU_SECTIONS) {
      const first = section.items.find(i => hasAccess(i.key));
      if (first) { navigate(first.path, { replace: true }); return; }
    }
  }, [location.pathname, loadingPerms, permissions, user]);

  const filteredSections = MENU_SECTIONS.map(section => {
    const items = section.items.filter(i => hasAccess(i.key));
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans overflow-x-hidden">
      ...
      <aside className={`w-64 bg-white border-r border-[#E5E7EB] flex flex-col fixed h-full z-50 shadow-xl md:shadow-sm transition-all duration-300 ${isSidebarOpen ? 'left-0' : '-left-64 md:left-0'}`}>
          ...
          {loadingPerms ? (
            ...
            <>
              ...
              {filteredSections.map((section, idx) => (
                <div key={idx}>
                  <h4 className="text-[10px] font-bold text-gray-400 tracking-widest uppercase mb-2 px-3">
                    {section.title}
                  </h4>
        ...
        <div className="p-4 border-t border-[#E5E7EB]">
          <Link
            to="/profile"
            onClick={() => setIsSidebarOpen(false)}
            className="flex items-center gap-3 text-gray-600 font-semibold px-3 py-2.5 w-full hover:bg-gray-50 rounded-xl transition-colors"
          >
            <UserCircle size={18} />
            Profile Settings
          </Link>
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <div className="relative" ref={dropdownRef}>
              <div
                ...
                className="flex items-center gap-3 cursor-pointer pl-4 md:border-l border-[#E5E7EB] hover:opacity-80 transition-opacity"
              >
                <img
                  src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=2563EB&color=fff&rounded=true`}
                  alt="User"
                  className="w-8 h-8 rounded-lg md:rounded-full"
                />
      ...
      {/* MOBILE BOTTOM NAV */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#F8FAFC] border-t border-[#E5E7EB] flex justify-between items-center px-6 py-2 z-40">
        {hasAccess('dashboard') && (
        ...
        {hasAccess('bookings') && (
          <Link to="/bookings" className="flex flex-col items-center justify-center -mt-4 relative">
            ...
            <span className={`text-[10px] font-bold mt-1 ${location.pathname === '/bookings' ? 'text-[#2563EB]' : 'text-gray-500'}`}>Bookings</span>
          </Link>
        )}
