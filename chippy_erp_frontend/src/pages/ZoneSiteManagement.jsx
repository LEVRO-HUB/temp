...
export default function ZoneSiteManagement() {
  ...
  useEffect(() => {
    ...
    const handleSiteChange = (data) => {
      console.log('Real-time site update:', data);
      if (activeTab === 'sites') {
        fetchData();
      }
    };

    socket.on('zone_data_changed', handleZoneChange);
    socket.on('site_data_changed', handleSiteChange);

    return () => {
      socket.off('zone_data_changed', handleZoneChange);
      socket.off('site_data_changed', handleSiteChange);
    };
  }, [activeTab]);
  ...
  const handleEditZone = (z, viewOnly = false) => {
    setSearchParams({ mode: 'createZone', id: z.id.toString(), view: viewOnly ? 'true' : 'false' });
  };

  const handleEditSite = (s, viewOnly = false) => {
    setSearchParams({ mode: 'createSite', id: s.id.toString(), view: viewOnly ? 'true' : 'false' });
  };

  const resetZoneForm = () => { setSearchParams({ mode: 'createZone' }); };
  const resetSiteForm = () => { setSearchParams({ mode: 'createSite' }); };
