...
export default function ScreenRights() {
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-4 md:p-0 space-y-8 animate-in fade-in duration-500">
      {/* HEADER */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="flex justify-center items-center gap-2 bg-[#0F172A] text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 w-full md:w-auto">
          <Plus size={18} />
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
             <div className="relative w-full md:w-auto">
                ...
                <input 
                    ...
                    className="pl-10 pr-4 py-2 w-full md:w-64 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                />
             </div>
             <div className="flex gap-2 w-full md:w-auto">
                 <button onClick={fetchRoles} className="flex-1 md:flex-none flex justify-center p-2 text-gray-500 hover:bg-gray-50 rounded-lg border border-gray-100 transition-colors">
                    <Loader2 size={18} className={loading ? 'animate-spin' : ''} />
                 </button>
                 <button className="flex-1 md:flex-none flex justify-center p-2 text-gray-500 hover:bg-gray-50 rounded-lg border border-gray-100 transition-colors"><Filter size={18} /></button>
                 <button className="flex-1 md:flex-none flex justify-center p-2 text-gray-500 hover:bg-gray-50 rounded-lg border border-gray-100 transition-colors"><Download size={18} /></button>
             </div>
          </div>
        </div>
                       ...
                       <div className="flex justify-between items-start">
                          <div className="flex items-center gap-3">
                              ...
                              <div>
                                  ...
                                  <p className="text-xs font-mono text-gray-500 tracking-tighter mt-0.5">ROLE_{String(role.id).padStart(3, '0')}</p>
                              </div>
                          </div>
                          <span className={`px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border ${levelInfo.color}`}>
                             {levelInfo.level}
                          </span>
                       </div>
                       <div className="flex justify-end items-center gap-2 mt-2 pt-3 border-t border-gray-50">
                          <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Edit2 size={16} /></button>
                          <button className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                          <button 
                            onClick={() => navigate(`/rbac/permissions/${role.id}`)}
                            className="flex items-center gap-1 p-2 px-3 text-sm font-bold text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
                          >
                            Permissions <MoreVertical size={14} />
                          </button>
                       </div>
                    </div>
                  );
